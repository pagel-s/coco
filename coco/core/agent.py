...
class Agent:
    ...
    def _init_vector_db(self) -> None:
        try:
            ...
            collection_name = f"agent_{self.agent_id}_gen_{self.generation}".replace("-", "_").replace(".", "_")
            self._vector_collection = client.get_or_create_collection(name=collection_name)
        except ImportError:
    ...
    async def _query_vector_memory(self, query_text: str) -> List[str]:
        ...
        results = self._vector_collection.query(
            ...
            n_results=self.memory_config.top_k
        )
        return results["documents"][0] if results["documents"] else []

    def _build_system_prompt(self, relevant_memories: Optional[List[str]] = None) -> str:
...
"""
    ...
    async def act(self, environment_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        state_prompt = json.dumps({
            ...
            "recent_memory": self.memory[-5:] # Always show last 5 turns as working memory
        }, indent=2)
        ...
        try:
            ...
            mem_entry = {"state_hash": hash(str(environment_state)), "reasoning": reasoning}
