        ...
        if not gen_interactions.empty:
            ...
            for _, row in gen_interactions.iterrows():
                if G.has_edge(row['source_id'], row['target_id']):
                    G[row['source_id']][row['target_id']]['weight'] += 1
                else:
                    G.add_edge(row['source_id'], row['target_id'], weight=1, type=row['interaction_type'])
            ...
            st.dataframe(gen_interactions[['source_id', 'target_id', 'interaction_type', 'success', 'reasoning']].dropna(), use_container_width=True)
        else:
