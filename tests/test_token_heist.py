...
def agent_a() -> Agent:
    a = Agent(
        agent_id="agent_a",
        traits=AgentTraits(aggression_threshold=1.0, trust_level=0.0),
    )
    return a
...
def agent_b() -> Agent:
    b = Agent(
        agent_id="agent_b",
        traits=AgentTraits(aggression_threshold=0.0, trust_level=1.0),
    )
    return b
...
def test_get_agent_view(
    env: TokenHeistEnvironment, agent_a: Agent, agent_b: Agent
) -> None:
    env.register_agent(agent_a)
...
@pytest.mark.asyncio
async def test_attempt_theft_failure_dead_agents(
    env: TokenHeistEnvironment, agent_a: Agent, agent_b: Agent
) -> None:
    env.register_agent(agent_a)
    env.register_agent(agent_b)
...
@pytest.mark.asyncio
async def test_attempt_theft_success(
    env: TokenHeistEnvironment, agent_a: Agent, agent_b: Agent
) -> None:
    env.register_agent(agent_a)
    env.register_agent(agent_b)
