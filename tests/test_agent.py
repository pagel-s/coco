...
def agent() -> Agent:
    traits = AgentTraits(
        collaboration_threshold=0.8, aggression_threshold=0.2, trust_level=0.9
    )
...
@pytest.mark.asyncio
async def test_agent_share_failure_no_resource(
    agent: Agent, dummy_env: DummyEnvironment
) -> None:
