...
def test_get_agent_view(env: NumberGuesserEnvironment, agent: Agent) -> None:
    ...
    # Check feedback
    assert view["personal_feedback"] == "No guesses made yet. Guess a number between 1 and 100."
...
@pytest.mark.asyncio
async def test_handle_custom_action_correct_guess(env: NumberGuesserEnvironment, agent: Agent) -> None:
    env.register_agent(agent)
...
@pytest.mark.asyncio
async def test_handle_custom_action_low_guess(env: NumberGuesserEnvironment, agent: Agent) -> None:
    env.register_agent(agent)
...
@pytest.mark.asyncio
async def test_handle_custom_action_high_guess(env: NumberGuesserEnvironment, agent: Agent) -> None:
    env.register_agent(agent)
...
@pytest.mark.asyncio
async def test_handle_custom_action_invalid_guess_str(env: NumberGuesserEnvironment, agent: Agent) -> None:
    env.register_agent(agent)
    action = {"action_type": "guess", "value": "42"}
    result = await env.handle_custom_action("test_agent", action)
...
@pytest.mark.asyncio
async def test_handle_custom_action_invalid_type(env: NumberGuesserEnvironment, agent: Agent) -> None:
    env.register_agent(agent)
...
@pytest.mark.asyncio
async def test_handle_custom_action_unknown_action(env: NumberGuesserEnvironment, agent: Agent) -> None:
    env.register_agent(agent)
