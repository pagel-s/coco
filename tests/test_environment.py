...
async def test_transfer_resource_success(
    env: Environment, agent_a: Agent, agent_b: Agent
) -> None:
    env.register_agent(agent_a)
    env.register_agent(agent_b)
...
@pytest.mark.asyncio
async def test_transfer_resource_failure(
    env: Environment, agent_a: Agent, agent_b: Agent
) -> None:
    env.register_agent(agent_a)
    env.register_agent(agent_b)
...
@pytest.mark.asyncio
async def test_attempt_theft_failure_no_resource(
    env: Environment, agent_a: Agent, agent_b: Agent
) -> None:
    env.register_agent(agent_a)
    env.register_agent(agent_b)
...
async def test_execute_action_share(
    ...
    action = {"action_type": "share", "target_id": "agent_b", "resource_key": "token"}
