...
def main() -> None:
    ...
    if not sims.empty:
        sim_id = st.sidebar.selectbox(
            "Select Simulation ID", sims["simulation_id"].unique()
        )
        sim_config = json.loads(
            sims[sims["simulation_id"] == sim_id]["config_json"].iloc[0]
        )
    ...
    with tab_overview:
        ...
        col1.metric(
            "Generations",
            agents[agents["simulation_id"] == sim_id]["generation"].max() + 1,
        )
        ...
        col4.metric(
            ...
            len(
                interactions[
                    interactions["turn_id"].isin(
                        turns[turns["simulation_id"] == sim_id]["turn_id"]
                    )
                ]
            ),
        )
        ...
        # Population Summary
        avg_traits = (
            agents[agents["simulation_id"] == sim_id]
            .groupby("generation")[
                ["collaboration_threshold", "aggression_threshold", "trust_level"]
            ]
            .mean()
            .reset_index()
        )
        fig_pop = px.line(
            avg_traits,
            x="generation",
            y=["collaboration_threshold", "aggression_threshold", "trust_level"],
            title="Average Trait Evolution",
            labels={"value": "Level", "variable": "Trait"},
        )
    ...
    with tab_evolution:
        ...
        fitness_data = fitness_data.merge(
            agents[
                [
                    "agent_id",
                    "generation",
            ...
            on=["agent_id", "generation"],
        )

        fig_fit = px.scatter(
            fitness_data,
            x="aggression_threshold",
            y="fitness",
            color="generation",
            size="collaboration_threshold",
            hover_name="agent_id",
    ...
    with tab_interactions:
        ...
        gen_to_show = st.slider(
            "Select Generation for Network View", 0, int(agents["generation"].max()), 0
        )

        # Filter interactions for selected generation
        gen_turns = turns[
            (turns["simulation_id"] == sim_id) & (turns["generation"] == gen_to_show)
        ]["turn_id"]
        ...
        if not gen_interactions.empty:
            ...
            for _, row in gen_interactions.iterrows():
                ...
                if source and target and str(target).strip():
                    if G.has_edge(source, target):
                        ...
                        G.add_edge(
                            source, target, weight=1, type=row["interaction_type"]
                        )
                elif source:
            ...
            if len(G.nodes()) > 0:
                ...
                fig_graph = go.Figure(
                    ...
                    layout=go.Layout(
                        title=f"Interaction Network - Generation {gen_to_show}",
                        showlegend=False,
                        hovermode="closest",
                ...
                st.info(
                    "No directed interactions (stealing/sharing) to display in the graph for this generation."
                )
    ...
    with tab_agents:
        ...
        agent_info = agents[
            (agents["agent_id"] == selected_agent) & (agents["simulation_id"] == sim_id)
        ].iloc[0]
        ...
        with col_b:
            st.subheader("Traits")
            st.progress(
                agent_info["collaboration_threshold"],
                text=f"Collaboration: {agent_info['collaboration_threshold']:.2f}",
            )
            st.progress(
                agent_info["aggression_threshold"],
                text=f"Aggression: {agent_info['aggression_threshold']:.2f}",
            )
            st.progress(
                agent_info["trust_level"],
                text=f"Trust: {agent_info['trust_level']:.2f}",
            )

        st.subheader("Performance & Resources over Turns")
        agent_snaps = snapshots[snapshots["agent_id"] == selected_agent].merge(
            turns[["turn_id", "turn_number"]], on="turn_id"
        )
        agent_snaps["tokens"] = agent_snaps["resources_json"].apply(
            lambda x: json.loads(x).get("token", 0)
        )
        ...
        fig_tokens = px.line(
            ...
            x="turn_number",
            y="tokens",
            title=f"Token Balance History - {selected_agent}",
        )
        st.plotly_chart(fig_tokens, use_container_width=True)

        st.subheader("Private Reasoning Trace")
        st.dataframe(
            agent_snaps[["turn_number", "memory_json", "fitness"]],
            use_container_width=True,
        )
